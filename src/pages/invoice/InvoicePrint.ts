...
export function printInvoice({ invoice, storeName, storePhone = '', storeAddress = '' }: PrintInvoiceOptions): void {
  const itemRows = invoice.items
    .map(i => `<tr>
      <td style="padding:5px 0;border-bottom:1px solid #F5F4F0;font-size:12px">${i.description}</td>
      <td style="padding:5px 0;border-bottom:1px solid #F5F4F0;font-size:12px;text-align:center">${i.qty}</td>
      <td style="padding:5px 0;border-bottom:1px solid #F5F4F0;font-size:12px;text-align:right">${formatPHP(i.price)}</td>
      <td style="padding:5px 0;border-bottom:1px solid #F5F4F0;font-size:12px;text-align:right">${formatPHP(i.price * i.qty)}</td>
    </tr>`)
    .join('')
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: Arial, sans-serif; font-size: 13px; max-width: 400px; margin: 0 auto; padding: 20px; color: #1A1917; }
    .header { text-align: center; margin-bottom: 16px; padding-bottom: 12px; border-bottom: 2px solid #E01C24; }
    .store-name { font-size: 20px; font-weight: bold; }
    .store-info { font-size: 11px; color: #6B6760; margin-top: 2px; }
    .inv-no { color: #E01C24; font-weight: bold; font-size: 14px; margin: 8px 0 4px; }
    .inv-date { font-size: 11px; color: #6B6760; }
    ...
    .total-section { margin-top: 8px; padding-top: 8px; border-top: 1px solid #E8E5DF; }
    .total-row { display: flex; justify-content: space-between; font-size: 13px; margin-bottom: 4px; }
    .grand-total { font-weight: bold; font-size: 16px; color: #E01C24; }
    .paid-via { font-size: 12px; color: #6B6760; margin-top: 8px; }
    .footer { margin-top: 16px; text-align: center; font-size: 11px; color: #6B6760; border-top: 1px solid #E8E5DF; padding-top: 12px; }
